...
//            }
    ...
    /**Adjust a connector by a ConnectionPoint id
     * The ConnectionPoint should be already changed/transformed
     * Usually the connector changed it's start or end ConnectionPoint and now we need to update it's turning
     * points (intermediate points)
     *@param {Number} cpId - the id of the {ConnectionPoint} of the {Connector}
     *@param {Number} x - not used
     *@param {Number} y - not used
     *@deprecated
     **/
    deprecated_connectorAdjustByConnectionPoint: function(cpId, x, y){
        var cp = this.connectionPointGetById(cpId); //ConnectionPoint
        Log.debug("connectorAdjustByConnectionPoint() - Cp is :" + cp);
        var con = this.connectorGetById(cp.parentId); //Connector
        var conCps = this.connectionPointGetAllByParent(con.id); //Conector's ConnectionPoints
        ...
        if(con.type == Connector.TYPE_STRAIGHT){
            ...
            //START FIGURE - Do we have one?
            var glue  = CONNECTOR_MANAGER.glueGetBySecondConnectionPointId(conCps[0].id)[0];//the (only) Glue tied to ConnectionPoint
            if(glue != null){ //only if there is a Figure glued
                /**To find the exit point we will go N, E, S and W and move away from the Figure's
                 *bound by 10 pixels.
                ...
                //north
                tempExit = new Point(start.x, fb[1] - FIGURE_ESCAPE_DISTANCE);
                startExitPoint = tempExit;

                //east
                tempExit = new Point(fb[2] + FIGURE_ESCAPE_DISTANCE, start.y);
                if(Util.distance(start, tempExit) < Util.distance(start, startExitPoint)){
                    startExitPoint = tempExit;
                }

                //south
                tempExit = new Point(start.x, fb[3] + FIGURE_ESCAPE_DISTANCE);
                if(Util.distance(start, tempExit) < Util.distance(start, startExitPoint)){
                    startExitPoint = tempExit;
                }
                
                //west
                tempExit = new Point(fb[0] - FIGURE_ESCAPE_DISTANCE, start.y);
                if(Util.distance(start, tempExit) < Util.distance(start, startExitPoint)){
                    startExitPoint = tempExit;
                }                
            }
            ...
            //END FIGURE
            var glues = CONNECTOR_MANAGER.glueGetBySecondConnectionPointId(conCps[1].id);
            if(glues.length > 0){ //only if there is a Figure glued
                glue  = glues[0];//there will only be one for this
                Log.debug("connectorAdjustByConnectionPoint() - glue is " + glue);
                ...
                //north
                tempExit = new Point(end.x, fb[1] - FIGURE_ESCAPE_DISTANCE);
                endExitPoint = tempExit;

                //east
                tempExit = new Point(fb[2] + FIGURE_ESCAPE_DISTANCE, end.y);
                if(Util.distance(end, tempExit) < Util.distance(end, endExitPoint)){
                    endExitPoint = tempExit;
                }

                //south
                tempExit = new Point(end.x, fb[3] + FIGURE_ESCAPE_DISTANCE);
                if(Util.distance(end, tempExit) < Util.distance(end, endExitPoint)){
                    endExitPoint = tempExit;
                }

                //west
                tempExit = new Point(fb[0] - FIGURE_ESCAPE_DISTANCE, end.y);
                if(Util.distance(end, tempExit) < Util.distance(end, endExitPoint)){
                    endExitPoint = tempExit;
                }
            } else{
                Log.debug("connectorAdjustByConnectionPoint() - There is NO figure at the end");
            }

            
            var points = [start, end];
            var index = 0; //initially set to start

            //add start exit point
            if(startExitPoint != null){
                Log.debug("ConnectionManage:connectorAdjustByConnectionPoint() - startExitPoint:  " + startExitPoint)
                points.splice(index + 1,0, startExitPoint);
                index++;
            }

            //add end exit point
            if(endExitPoint != null){
                Log.debug("ConnectionManage:connectorAdjustByConnectionPoint() - endExitPoint:  " + endExitPoint)
                points.splice(index + 1, 0, endExitPoint);
                //endIndex = endIndex + 1;
            }
            ...
            if(points[index].x == points[index+1].x || points[index].y == points[index+1].y){
                //we have the S0 solution we do not have to add any new turning point
                //Log.info("S0 solution - no additional turning points");
                var intersection = false;
                if(startFigure != null){
                    if(Util.lineIntersectsRectangle(points[index], points[index+1], startFigure.getBounds())){
                        intersection = true;
                    }
                }

                if(endFigure != null){
                    if(Util.lineIntersectsRectangle(points[index], points[index+1], endFigure.getBounds())){
                        intersection = true;
                    }
                }

                if(!intersection){
                    //ok , there is no intersection so we have a solution
                    con.updateMiddleText();
                    return;
                }
            }
            ...
            //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            var impure = []; //vector of possible solutions (valid connector that might intersect a figure)
            
            //S1 - solution
            var s1 = null;
            
            var s1_1 = new Point(points[index+1].x, points[index].y); //first possible solution
            var p1_1 = Point.cloneArray(points);
            p1_1.splice(index+1, 0, s1_1);
            if(this._validPath(p1_1)){ //if valid collect to impure solutions
                impure.push(p1_1);
            }
            
            var s1_2 = new Point(points[index].x, points[index+1].y); //second possible solution
            var p1_2 = Point.cloneArray(points);
            p1_2.splice(index+1, 0, s1_2);
            if(this._validPath(p1_2)){ //if valid collect to impure solutions
                impure.push(p1_2);
            }
            
            var score1 = this._scorePath( p1_1, true );
            var score2 = this._scorePath( p1_2, true );
            
            //Log.info("Score 1_1: " + score1 + " score 2_1: " + score2);
            ...
            if(s1 != null){
                //Log.info("s1 : " + s1);
                /**Test so that the connector will not intersect any of the figures*/
                var intersection = false;
                var s1_points = Point.cloneArray(points);
                s1_points.splice(index + 1, 0, s1);
                //Log.info("s1_points has " + s1_points.length + " points");
                //
                //we will try to avoid useless intersection tests
                var startIndex = 1;
                var endIndex = s1_points.length-2;
                if(startFigure == null){
                    startIndex = 0;
                }
                if(endFigure == null){
                    endIndex = s1_points.length-1;
                }
                for(var p=startIndex; p < endIndex; p++){
                    if(startFigure != null){
                        if(Util.lineIntersectsRectangle(s1_points[p], s1_points[p+1], startFigure.getBounds())){
                            intersection = true;
                            break;
                        }
                    }
                    ...
                    if(endFigure != null){
                        if(Util.lineIntersectsRectangle(s1_points[p], s1_points[p+1], endFigure.getBounds())){
                            intersection = true;
                            break;
                        }
                    }
                ...
                if(!intersection){ //we will consider we have a solution ONLY if there is no intersection
                    con.turningPoints = s1_points;
                    //Log.info("S1 : " + s1_points);
                    con.updateMiddleText();
                    return;
                }

                //Log.info("con.turningPoints has " + con.turningPoints.length + " points");
            }
            else{
                //Log.info("S2 here we came");
            }


            //S2 - solution
            var p2Pure = []; //vector of pure solutions (valid connectors that do not intersects any figure)
            


            //I - variant
            var s2_I_1 = new Point( (points[index].x + points[index+1].x) / 2,  points[index].y);
            var s2_I_2 = new Point( (points[index].x + points[index+1].x) / 2,  points[index+1].y);

            var p2_I = Point.cloneArray(points);
            p2_I.splice(index + 1, 0, s2_I_1, s2_I_2);

            //Log.info("p2_I: " + p2_I);
            if(this._validPath(p2_I)){
                impure.push(p2_I);
                var I_intersect = false;
                for(var j=1;j<p2_I.length-2; j++){
                    if( (startFigure != null && Util.lineIntersectsRectangle(p2_I[j], p2_I[j+1], startFigure.getBounds()))
                        ||
                        (endFigure != null && Util.lineIntersectsRectangle(p2_I[j], p2_I[j+1], endFigure.getBounds()))
                    )
                    {
                        I_intersect = true;
                        break;
                ...
                Log.debug("S2 I valid");
            }
            else{
                Log.debug("S2 I not valid");
            }
            

            //II - variant
            var s2_II_1 = new Point( points[index].x, (points[index].y + points[index+1].y)/2 );
            var s2_II_2 = new Point( points[index+1].x, (points[index].y + points[index+1].y)/2);

            var p2_II = Point.cloneArray(points);
            p2_II.splice(index + 1, 0, s2_II_1, s2_II_2);

            //Log.info("p2_II: " + p2_II);
            if(this._validPath(p2_II)){
                impure.push(p2_II);
                var II_intersect = false;
                for(var j=1;j<p2_II.length-2; j++){ //we will not seach for intersections with the extremities of the connector (ex: 0 to 1 and 4 to 5)
                    if( (startFigure != null && Util.lineIntersectsRectangle(p2_II[j], p2_II[j+1], startFigure.getBounds()))
                        ||
                        (endFigure != null && Util.lineIntersectsRectangle(p2_II[j], p2_II[j+1], endFigure.getBounds()))
                    )
                    {
                        II_intersect = true;
                        break;
                ...
                Log.debug("S2 II valid");
            }
            else{
                Log.debug("S2 II not valid");
            }
            ...
            var eastExits = [];
            if(startFigure != null){
                eastExits.push(startFigure.getBounds()[2] + 20);
            }

            if(endFigure != null){
                eastExits.push(endFigure.getBounds()[2] + 20);
            }

            if(eastExits.length != 0){
                var eastExit = Util.max(eastExits); //max east

                var s2_III_1 = new Point(eastExit, points[index].y);
                var s2_III_2 = new Point(eastExit, points[index+1].y);

                var p2_III =  Point.cloneArray(points);
                p2_III.splice(index + 1, 0, s2_III_1, s2_III_2);

                //Log.info("p2_III: " + p2_III);
                if(this._validPath(p2_III)){
                    impure.push(p2_III);
                    
                    Log.debug("S2 III valid");

                    var III_intersect = false;
                    for(var j=1;j<p2_III.length-2; j++){ //we will not seach for intersections with the extremities of the connector (ex: 0 to 1 and 4 to 5)
                        if( (startFigure != null && Util.lineIntersectsRectangle(p2_III[j], p2_III[j+1], startFigure.getBounds()))
                            ||
                            (endFigure != null && Util.lineIntersectsRectangle(p2_III[j], p2_III[j+1], endFigure.getBounds()))
                        )
                        {
                            III_intersect = true;
                            break;
                    ...
                    Log.debug("S2 III not valid");
                }
            }
            

            //IV (North)
            Log.debug("Start S2 IV solution");
            var northExits = [];
            if(startFigure != null){
                northExits.push(startFigure.getBounds()[1] - 20);
            }

            if(endFigure != null){
                northExits.push(endFigure.getBounds()[1] - 20);
            }

            Log.debug("S2 IV test North exit points");
            if(northExits.length != 0){
                Log.debug("S2 IV we have  North points");
                var northExit = Util.min(northExits); //northest point

                var s2_IV_1 = new Point(points[index].x, northExit);
                var s2_IV_2 = new Point(points[index+1].x, northExit);

                var p2_IV =  Point.cloneArray(points);
                p2_IV.splice(index + 1, 0, s2_IV_1, s2_IV_2);

                //Log.info("p2_IV: " + p2_IV);
                if(this._validPath(p2_IV)){
                    impure.push(p2_IV);
                    Log.debug("S2 IV valid");

                    var IV_intersect = false;
                    for(var j=1;j<p2_IV.length-2; j++){ //we will not seach for intersections with the extremities of the connector (ex: 0 to 1 and 4 to 5)
                        if( (startFigure != null && Util.lineIntersectsRectangle(p2_IV[j], p2_IV[j+1], startFigure.getBounds()))
                            ||
                            (endFigure != null && Util.lineIntersectsRectangle(p2_IV[j], p2_IV[j+1], endFigure.getBounds()))
                        )
                        {
                            IV_intersect = true;
                            break;
                    ...
                    if(!IV_intersect){
                        p2Pure.push(p2_IV);
                        Log.debug("S2 IV pure");
                    }
                    else{
                        Log.debug("S2 IV NOT pure");
                    }
                }
                else{
                    Log.debug("S2 IV not valid");
                }
            } else{
                Log.debug("No North (S2 IV) points");
            }
            ...
            var westExits = [];
            if(startFigure != null){
                westExits.push(startFigure.getBounds()[0] - 20);
            }

            if(endFigure != null){
                eastExits.push(endFigure.getBounds()[0] - 20);
            }

            if(westExits.length != 0){
                var westExit = Util.min(westExits); //westest point

                var s2_V_1 = new Point(westExit, points[index].y);
                var s2_V_2 = new Point(westExit, points[index+1].y);

                var p2_V =  Point.cloneArray(points);
                p2_V.splice(index + 1, 0, s2_V_1, s2_V_2);

                //Log.info("p2_V: " + p2_V);
                if(this._validPath(p2_V)){
                    impure.push(p2_V);
                    Log.debug("S2 V valid");

                    var V_intersect = false;
                    for(var j=1;j<p2_V.length-2; j++){ //we will not seach for intersections with the extremities of the connector (ex: 0 to 1 and 4 to 5)
                        if( (startFigure != null && Util.lineIntersectsRectangle(p2_V[j], p2_V[j+1], startFigure.getBounds()))
                            ||
                            (endFigure != null && Util.lineIntersectsRectangle(p2_V[j], p2_V[j+1], endFigure.getBounds()))
                        )
                        {
                            V_intersect = true;
                            break;
            ...
            var southExits = [];
            if(startFigure != null){
                southExits.push(startFigure.getBounds()[3] + 20);
            }

            if(endFigure != null){
                southExits.push(endFigure.getBounds()[3] + 20);
            }

            if(southExits.length != 0){
                var southExit = Util.max(southExits); //southest y coordinates (max y)

                var s2_VI_1 = new Point(points[index].x, southExit);
                var s2_VI_2 = new Point(points[index+1].x, southExit);

                var p2_VI =  Point.cloneArray(points);
                p2_VI.splice(index + 1, 0, s2_VI_1, s2_VI_2);

                //Log.info("p2_VI: " + p2_VI);
                if(this._validPath(p2_VI)){
                    impure.push(p2_VI);
                    Log.debug("S2 VI valid");

                    var VI_intersect = false;
                    for(var j=1;j<p2_VI.length-2; j++){ //we will now seach for intersections with figures (potins 0 to 1 and 4 to 5 will be ignored)
                        if( (startFigure != null && Util.lineIntersectsRectangle(p2_VI[j], p2_VI[j+1], startFigure.getBounds()))
                            ||
                            (endFigure != null && Util.lineIntersectsRectangle(p2_VI[j], p2_VI[j+1], endFigure.getBounds()))
                        )
                        {
                            VI_intersect = true;
                            break;
                    ...
                    Log.debug("S2 VI not valid");
                }
            }

            //sort solutions by their length
            //TODO: use a quick sort (in JavaScript library :D)
            if(p2Pure.length > 1){ //sort only when we have at least 1
                var sorted = false;
                while(!sorted){
                    sorted = true;
                    for(k=0;k<p2Pure.length-1; k++){
                        if(Util.getPolylineLength(p2Pure[k]) > Util.getPolylineLength(p2Pure[k+1])){
                            var temp = p2Pure[k];
                            p2Pure[k] = p2Pure[k+1]
                            p2Pure[k + 1] = temp;
                            sorted = false;
            ...
            Log.info("Pure solution : " + p2Pure.length +  " Impure solutions : " + impure.length);

            //pick a solution
            if(p2Pure.length > 0){
                con.turningPoints = p2Pure[0];
            }
            else{
                //TODO: maybe sort impure solutions based too ?!
                con.turningPoints = impure[0];
            }
