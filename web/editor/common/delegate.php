...
class Delegate {
	/*	 * Load a CSV file into a multidimensional array */
	...
	public static function userGetAll() {
		...
		$users = array();
		$usersArray = Delegate::loadCSV($usersPath);
	...
	public static function userGetByEmailAndPassword($email, $password) {
		$users = Delegate::userGetAll();
	...
	public static function userAdd($email, $password, $name) {
	...
	public static function userEdit($email, $password, $name) {
		$usersPath = dirname(__FILE__) . '/../data/users.txt';
	...
	public static function userRemove($email) {
		$usersPath = dirname(__FILE__) . '/../data/users.txt';
...
if (false && PHP_SAPI == 'cli') { //see http://php.net/manual/en/features.commandline.php
	...
	Delegate::userAdd('nana@brasov.ro', '123456');
//	userAdd('ioana@brasov.ro', '1234446', 'Ioana Popon');
	//userRemove('nana@brasov.ro');
	Delegate::userEdit('alex@scriptoid.com', '12121212', 'Alexandru Florin Gheorghiu');
}
?>